...
사용 방법
    ...
    1.트리거 저장
        ...
        1.2 Image, Mouse, Keyboard, RelativeToImage 이벤트 선택
            ...
            - Mouse : 좌표 지정(좌클릭, 우클릭, 드래그) 해주시면 됩니다.
        
            - Keyboard : Ctrl + c + v 이런식으로 조합키를 넣어주면 됩니다.

            - RelativeToImage : 찾은 이미지로부터 +- 한 거리를 클릭합니다.(아이디어 Ko9ma7 제공)
...
작업 목록
    ...
    8 에뮬레이터 사이즈 변경시 크기 비율에 따른 캡쳐 이미지 사이즈 변경(홀드)

    11.OpenCV ROI 기능 적용(홀드)
...
2.3.3
    ...
    1.반복 기능 추가 : 이미지 찾을 때 까지

    - 상위 아이템을 찾을 때까지 반복하는 기능입니다. 상위 아이템을 찾을때까지 하위 아이템을 반복해서 찾습니다.

    - 반복 제한 횟수만큼 계속 반복합니다. 제한만큼 반복하며 이후 다음 아이템을 검색합니다.

    2.이미지 이벤트 클릭 변경

    - 찾은 이미지 내에서 랜덤 클릭(클릭 이벤트 좌표 패턴을 분석 되는 것을 막습니다.)
